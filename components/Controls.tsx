...
import { Wand2, Download, AlertCircle, RefreshCw, Check, Loader2 } from 'lucide-react';
      ...
      <div className="p-5 space-y-8">
          ...
          <div className="space-y-3">
            <div className="relative">
              <textarea
                ...
                placeholder="Describe your edit (e.g., 'Change bg to blue', 'Fix hair')..."
                className="w-full p-3 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none resize-none h-24 bg-slate-50"
              />
              <button
                onClick={handleRunAI}
                disabled={isProcessing || !customPrompt.trim()}
                className="absolute bottom-2 right-2 p-1.5 bg-blue-600 text-white rounded-md disabled:opacity-50 hover:bg-blue-700 transition-colors shadow-sm"
              >
            ...
            <div className="flex flex-wrap gap-1.5">
              {SUGGESTED_PROMPTS.slice(0, 4).map((prompt, idx) => (
                <button
                  ...
                  className="text-xs px-2 py-1 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-md transition-colors text-left truncate max-w-full"
                >
                  {prompt}
                </button>
              ))}
        ...
        <section className="pt-4 border-t border-slate-100">
           <button
            ...
            className={`
              w-full py-3 rounded-lg flex items-center justify-center gap-2 font-medium transition-all shadow-sm
              ${hasProcessedImage 
